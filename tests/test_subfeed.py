...
async def test_subfeed_returns_data():
    ...
    result = await sf.execute(
        methods_dict=METHODS, session_id="s1", limit=5, cursor={}
    )
...
async def test_subfeed_stamps_debug_info():
    ...
    result = await sf.execute(
        methods_dict=METHODS, session_id="s1", limit=3, cursor={}
    )
...
async def test_subfeed_cursor_propagation():
    ...
    r2 = await sf.execute(
        methods_dict=METHODS, session_id="s1", limit=5, cursor=r1.next_page
    )
